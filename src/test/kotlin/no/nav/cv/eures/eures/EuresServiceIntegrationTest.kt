...
class EuresServiceIntegrationTest {
    ...
    fun `records skal endre type basert paa timestamps`() {
        ...
        val candidate = cvXmlRepository.save(CvXml().update("PAM-4", "1234567893", now, now, null, xml = "SOME XML", checksum = "SOME CHECKSUM", null))
        ...
        cvXmlRepository.save(candidate.update(candidate.reference, candidate.foedselsnummer, candidate.opprettet, now, null, "SOME UPDATED XML", checksum = "SOME CHECKSUM", null))
        ...
        now = ZonedDateTime.now().plusHours(1)
        cvXmlRepository.save(candidate.update(candidate.reference, candidate.foedselsnummer, candidate.opprettet, now, now, "SOME UPDATED XML", checksum = "SOME CHECKSUM", null))
